...
"""
FOR `postprocessing_and_gaze` REPOSITORY
===
This script was copied from the `kromo` repository on GitHub
(https://github.com/yoonsik/kromo) with small modifications (changing all
instances of Image.ANTIALIAS to Image.LANCZOS, for the most recent version of
the PIL library). It is redistributed according to the terms of the GPL-3.0
license it was originally licensed under.
"""
...
def add_chromatic(im, strength: float = 1, no_blur: bool = False):
    ...
    gfinal = gfinal.resize((round((1 + 0.018 * strength) * rdata.shape[1]),
                            round((1 + 0.018 * strength) * rdata.shape[0])), Image.LANCZOS)
    bfinal = bfinal.resize((round((1 + 0.044 * strength) * rdata.shape[1]),
                            round((1 + 0.044 * strength) * rdata.shape[0])), Image.LANCZOS)
...
def blend_images(im, og_im, alpha: float = 1, strength: float = 1):
    ...
    og_im = og_im.resize((round((1 + 0.018 * strength) * og_im.size[0]),
                          round((1 + 0.018 * strength) * og_im.size[1])), Image.LANCZOS)
